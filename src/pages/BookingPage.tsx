...
export default function BookingPage() {
  ...
  const [upcomingBookings, setUpcomingBookings] = useState<BookingsData | null>(
    null
  );
  ...
  const sortBookings = () => {
    ...
    const upcoming = bookingData.filter(
      (booking) => booking.pick_up_date >= currentDate
    );
    const past = bookingData.filter(
      (booking) => booking.pick_up_date < currentDate
    );
    const sortedUpcoming = upcoming.sort((a, b) =>
      a.pick_up_date.localeCompare(b.pick_up_date)
    );
    const sortedPast = past.sort((a, b) =>
      b.pick_up_date.localeCompare(a.pick_up_date)
    );

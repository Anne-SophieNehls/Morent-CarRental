...
export default function HomePage(){
  ...
  const getVehicles = async () => {
    const result = await supabase.from("vehicles").select("*").ilike("brand", `%${searchFor}%`);
    return result;
  };
			...
			<div className="flex flex-col w-11/12">
				...
				<div className="flex justify-around mb-10 items-center">
						...
						<div className="flex">
							<div>
								...
								<select>
									...
									{locationsArray?.map((el, index) => <option key={index} value={el}>{el}</option>)};
								</select>
							</div>
								<div>
									<p>Date</p>
					...
					<div className="text-right mr-24 mb-5">
						<Button onSubmit={handleSubmit} className="bg-[#3563E9]">Filter</Button>
					</div>
					<div>
